# ============================================================
# PROJECT 02
# PORTFOLIO RISK & PERFORMANCE ANALYTICS
# ============================================================
#
# OBJECTIVE
# ------------------------------------------------------------
# Analyse the risk and performance of two frozen portfolios
# produced by Project 01:
#
#   1. Equal-weight benchmark
#   2. Max-40 constrained alternative model
#
# Project 02 does NOT:
#
#   - select assets
#   - calculate factors
#   - optimize parameters
#   - optimize portfolio weights
#
# It consumes the outputs of Project 01 and performs:
#
#   - Portfolio performance analysis
#   - Active return analysis
#   - Active weight analysis
#   - Tracking error
#   - Information ratio
#   - Portfolio risk contribution
#   - Performance attribution
#
# ============================================================


# ============================================================
# 1. IMPORT LIBRARIES
# ============================================================

import os

import numpy as np
import pandas as pd


# ============================================================
# 2. PROJECT SETTINGS
# ============================================================

TRADING_DAYS = 252

RISK_FREE_RATE = 0.0


# ============================================================
# 3. PROJECT 01 OUTPUT PATH
# ============================================================

PROJECT_01_OUTPUT = (
    "../project_01_asset_selection/outputs"
)


PROJECT_02_OUTPUT = (
    "../outputs"
)


os.makedirs(
    PROJECT_02_OUTPUT,
    exist_ok=True
)


# ============================================================
# 4. FILE PATHS
# ============================================================

PROCESSED_PRICES_FILE = os.path.join(
    PROJECT_01_OUTPUT,
    "processed_prices.csv"
)


FINAL_TEST_RETURNS_FILE = os.path.join(
    PROJECT_01_OUTPUT,
    "final_test_returns.csv"
)


FROZEN_PORTFOLIOS_FILE = os.path.join(
    PROJECT_01_OUTPUT,
    "frozen_portfolios.csv"
)


FINAL_OOS_RESULTS_FILE = os.path.join(
    PROJECT_01_OUTPUT,
    "final_oos_results.csv"
)


# ============================================================
# 5. CHECK PROJECT 01 OUTPUTS
# ============================================================

required_files = [

    PROCESSED_PRICES_FILE,

    FINAL_TEST_RETURNS_FILE,

    FROZEN_PORTFOLIOS_FILE,

    FINAL_OOS_RESULTS_FILE

]


for file_path in required_files:

    if not os.path.exists(file_path):

        raise FileNotFoundError(
            f"Required Project 01 output not found: "
            f"{file_path}"
        )


print(
    "\n" + "=" * 70
)

print(
    "PROJECT 02 — LOADING PROJECT 01 OUTPUTS"
)

print(
    "=" * 70
)


# ============================================================
# 6. LOAD CLEANED PRICES
# ============================================================
#
# This demonstrates that Project 02 receives cleaned data
# from Project 01 rather than repeating raw-data preparation.
# ============================================================

processed_prices = pd.read_csv(

    PROCESSED_PRICES_FILE,

    index_col=0,

    parse_dates=True

)


print(
    "\nProcessed prices loaded:"
)

print(
    processed_prices.shape
)


# ============================================================
# 7. LOAD FINAL OOS RETURNS
# ============================================================
#
# These are the returns of the selected assets during the
# genuinely unseen test period identified by Project 01.
# ============================================================

final_test_returns = pd.read_csv(

    FINAL_TEST_RETURNS_FILE,

    index_col=0,

    parse_dates=True

)


print(
    "\nFinal OOS returns loaded:"
)

print(
    final_test_returns.shape
)


print(
    "\nFinal OOS period:"
)

print(
    final_test_returns.index.min(),
    "to",
    final_test_returns.index.max()
)


# ============================================================
# 8. LOAD FROZEN PORTFOLIOS
# ============================================================

frozen_portfolios = pd.read_csv(

    FROZEN_PORTFOLIOS_FILE,

    index_col=0

)


print(
    "\nFrozen portfolios loaded:"
)

print(
    frozen_portfolios
)


# ============================================================
# 9. LOAD PROJECT 01 OOS RESULTS
# ============================================================
#
# This is used for cross-checking that Project 02 is analysing
# the same portfolios that Project 01 evaluated.
# ============================================================

project_01_oos_results = pd.read_csv(
    FINAL_OOS_RESULTS_FILE
)


print(
    "\nProject 01 OOS results loaded:"
)

print(
    project_01_oos_results
)


# ============================================================
# 10. IDENTIFY THE TWO PORTFOLIOS
# ============================================================

required_portfolios = [
    "Equal_Weight",
    "Max_40"
]


for portfolio in required_portfolios:

    if portfolio not in frozen_portfolios.index:

        raise ValueError(
            f"{portfolio} is missing from frozen portfolios."
        )


benchmark_weights = (
    frozen_portfolios
    .loc["Equal_Weight"]
    .astype(float)
)


alternative_weights = (
    frozen_portfolios
    .loc["Max_40"]
    .astype(float)
)


# ============================================================
# 11. ALIGN WEIGHTS WITH RETURNS
# ============================================================

common_assets = (
    final_test_returns.columns
    .intersection(
        benchmark_weights.index
    )
)


if len(common_assets) == 0:

    raise ValueError(
        "No common assets between Project 01 returns "
        "and frozen portfolio weights."
    )


final_test_returns = (
    final_test_returns[
        common_assets
    ]
)


benchmark_weights = (
    benchmark_weights[
        common_assets
    ]
)


alternative_weights = (
    alternative_weights[
        common_assets
    ]
)


# ============================================================
# 12. WEIGHT CHECK
# ============================================================

if not np.isclose(
    benchmark_weights.sum(),
    1.0
):

    raise ValueError(
        "Equal-weight portfolio does not sum to 1."
    )


if not np.isclose(
    alternative_weights.sum(),
    1.0
):

    raise ValueError(
        "Alternative portfolio does not sum to 1."
    )


print(
    "\n" + "=" * 70
)

print(
    "PORTFOLIO DEFINITIONS"
)

print(
    "=" * 70
)

print(
    "\nBenchmark — Equal Weight:"
)

print(
    benchmark_weights
)


print(
    "\nAlternative — Max 40:"
)

print(
    alternative_weights
)


# ============================================================
# 13. PORTFOLIO RETURNS
# ============================================================

benchmark_returns = (
    final_test_returns
    .dot(
        benchmark_weights
    )
)


alternative_returns = (
    final_test_returns
    .dot(
        alternative_weights
    )
)


# ============================================================
# 14. BASIC PERFORMANCE FUNCTION
# ============================================================

def evaluate_portfolio(
    portfolio_returns
):
    """
    Calculate absolute portfolio performance metrics.
    """

    portfolio_returns = (
        portfolio_returns
        .dropna()
    )


    total_return = (
        (1 + portfolio_returns)
        .prod()
        - 1
    )


    annualized_return = (
        (1 + total_return)
        ** (
            TRADING_DAYS
            / len(portfolio_returns)
        )
        - 1
    )


    annualized_volatility = (
        portfolio_returns.std()
        * np.sqrt(TRADING_DAYS)
    )


    if annualized_volatility != 0:

        sharpe = (
            annualized_return
            - RISK_FREE_RATE
        ) / annualized_volatility

    else:

        sharpe = np.nan


    wealth = (
        1 + portfolio_returns
    ).cumprod()


    drawdown = (
        wealth
        /
        wealth.cummax()
        - 1
    )


    maximum_drawdown = (
        drawdown.min()
    )


    return {

        "Total_Return":
            total_return,

        "Annualized_Return":
            annualized_return,

        "Annualized_Volatility":
            annualized_volatility,

        "Sharpe_Ratio":
            sharpe,

        "Maximum_Drawdown":
            maximum_drawdown,

        "Final_Wealth":
            wealth.iloc[-1]

    }


# ============================================================
# 15. PERFORMANCE COMPARISON
# ============================================================

benchmark_metrics = (
    evaluate_portfolio(
        benchmark_returns
    )
)


alternative_metrics = (
    evaluate_portfolio(
        alternative_returns
    )
)


performance_comparison = pd.DataFrame(

    [

        {
            "Portfolio":
                "Equal_Weight",
            **benchmark_metrics
        },

        {
            "Portfolio":
                "Max_40",
            **alternative_metrics
        }

    ]

)


print(
    "\n" + "=" * 70
)

print(
    "PORTFOLIO PERFORMANCE COMPARISON"
)

print(
    "=" * 70
)

print(
    performance_comparison
)


# ============================================================
# 16. ACTIVE RETURN
# ============================================================
#
# Active return:
#
#       R_A = R_P - R_B
#
# where:
#
#       R_P = alternative portfolio return
#       R_B = equal-weight benchmark return
# ============================================================

active_returns = (
    alternative_returns
    - benchmark_returns
)


active_total_return = (
    (1 + active_returns)
    .prod()
    - 1
)


annualized_active_return = (
    active_returns.mean()
    * TRADING_DAYS
)


print(
    "\n" + "=" * 70
)

print(
    "ACTIVE RETURN ANALYSIS"
)

print(
    "=" * 70
)

print(
    "\nAlternative total return:",
    alternative_metrics["Total_Return"]
)

print(
    "Benchmark total return:",
    benchmark_metrics["Total_Return"]
)

print(
    "Active return:",
    active_total_return
)

print(
    "Annualized active return:",
    annualized_active_return
)


# ============================================================
# 17. ACTIVE WEIGHTS
# ============================================================

active_weights = (
    alternative_weights
    - benchmark_weights
)


active_weight_table = pd.DataFrame({

    "Alternative_Weight":
        alternative_weights,

    "Benchmark_Weight":
        benchmark_weights,

    "Active_Weight":
        active_weights

})


print(
    "\n" + "=" * 70
)

print(
    "ACTIVE WEIGHTS"
)

print(
    "=" * 70
)

print(
    active_weight_table
)

print(
    "\nActive weight sum:",
    active_weights.sum()
)


# ============================================================
# 18. TRACKING ERROR
# ============================================================
#
# Tracking error is the standard deviation of active returns.
# ============================================================

daily_tracking_error = (
    active_returns.std()
)


annualized_tracking_error = (
    daily_tracking_error
    * np.sqrt(TRADING_DAYS)
)


print(
    "\n" + "=" * 70
)

print(
    "TRACKING ERROR"
)

print(
    "=" * 70
)

print(
    "Daily tracking error:",
    daily_tracking_error
)

print(
    "Annualized tracking error:",
    annualized_tracking_error
)


# ============================================================
# 19. INFORMATION RATIO
# ============================================================
#
# IR = Annualized Active Return / Annualized Tracking Error
# ============================================================

if annualized_tracking_error != 0:

    information_ratio = (
        annualized_active_return
        / annualized_tracking_error
    )

else:

    information_ratio = np.nan


print(
    "\n" + "=" * 70
)

print(
    "INFORMATION RATIO"
)

print(
    "=" * 70
)

print(
    "Information Ratio:",
    information_ratio
)


# ============================================================
# 20. OOS COVARIANCE MATRIX
# ============================================================
#
# Project 02 calculates risk characteristics using the final
# unseen period.
# ============================================================

oos_covariance = (
    final_test_returns
    .cov()
    * TRADING_DAYS
)


print(
    "\n" + "=" * 70
)

print(
    "FINAL OOS ANNUALIZED COVARIANCE MATRIX"
)

print(
    "=" * 70
)

print(
    oos_covariance
)


# ============================================================
# 21. ALTERNATIVE PORTFOLIO RISK CONTRIBUTION
# ============================================================

alternative_weight_array = (
    alternative_weights.values
)


covariance_matrix = (
    oos_covariance
    .values
)


portfolio_variance = (
    alternative_weight_array
    @ covariance_matrix
    @ alternative_weight_array
)


portfolio_volatility = (
    np.sqrt(
        portfolio_variance
    )
)


sigma_w = (
    covariance_matrix
    @ alternative_weight_array
)


marginal_risk_contribution = (
    sigma_w
    / portfolio_volatility
)


component_risk_contribution = (
    alternative_weight_array
    * marginal_risk_contribution
)


risk_contribution_percentage = (
    component_risk_contribution
    / portfolio_volatility
)


risk_contribution = pd.DataFrame({

    "Weight":
        alternative_weight_array,

    "Marginal_Risk_Contribution":
        marginal_risk_contribution,

    "Component_Risk_Contribution":
        component_risk_contribution,

    "Risk_Contribution_%":
        risk_contribution_percentage
        * 100

},
index=common_assets
)


print(
    "\n" + "=" * 70
)

print(
    "ALTERNATIVE PORTFOLIO RISK CONTRIBUTION"
)

print(
    "=" * 70
)

print(
    risk_contribution
)

print(
    "\nRisk contribution sum:",
    risk_contribution_percentage.sum()
)


# ============================================================
# 22. PERFORMANCE ATTRIBUTION
# ============================================================
#
# Active contribution:
#
#       Active Weight × Asset Return
#
# This explains which active positions helped or hurt relative
# to the equal-weight benchmark.
# ============================================================

asset_oos_returns = (
    (
        final_test_returns
        + 1
    )
    .prod()
    - 1
)


active_contribution = (
    active_weights
    * asset_oos_returns
)


performance_attribution = pd.DataFrame({

    "Benchmark_Weight":
        benchmark_weights,

    "Alternative_Weight":
        alternative_weights,

    "Active_Weight":
        active_weights,

    "Asset_OOS_Return":
        asset_oos_returns,

    "Active_Contribution":
        active_contribution

})


print(
    "\n" + "=" * 70
)

print(
    "PERFORMANCE ATTRIBUTION"
)

print(
    "=" * 70
)

print(
    performance_attribution
)


print(
    "\nSum of active contributions:",
    active_contribution.sum()
)

print(
    "Actual active return:",
    active_total_return
)


# ============================================================
# 23. DAILY PERFORMANCE ATTRIBUTION
# ============================================================

daily_active_contribution = (
    final_test_returns
    .mul(
        active_weights,
        axis=1
    )
)


daily_active_contribution["Total_Active_Return"] = (
    daily_active_contribution
    .sum(axis=1)
)


# ============================================================
# 24. CREATE BENCHMARK VS ALTERNATIVE RETURN SERIES
# ============================================================

portfolio_returns = pd.DataFrame({

    "Equal_Weight":
        benchmark_returns,

    "Max_40":
        alternative_returns,

    "Active_Return":
        active_returns

})


# ============================================================
# 25. SAVE PROJECT 02 OUTPUTS
# ============================================================

performance_comparison.to_csv(

    os.path.join(
        PROJECT_02_OUTPUT,
        "performance_comparison.csv"
    ),

    index=False

)


active_weight_table.to_csv(

    os.path.join(
        PROJECT_02_OUTPUT,
        "active_weights.csv"
    )

)


performance_attribution.to_csv(

    os.path.join(
        PROJECT_02_OUTPUT,
        "performance_attribution.csv"
    )

)


risk_contribution.to_csv(

    os.path.join(
        PROJECT_02_OUTPUT,
        "risk_contribution.csv"
    )

)


portfolio_returns.to_csv(

    os.path.join(
        PROJECT_02_OUTPUT,
        "portfolio_returns.csv"
    )

)


daily_active_contribution.to_csv(

    os.path.join(
        PROJECT_02_OUTPUT,
        "daily_active_contribution.csv"
    )

)


oos_covariance.to_csv(

    os.path.join(
        PROJECT_02_OUTPUT,
        "oos_covariance_matrix.csv"
    )

)


# ============================================================
# 26. SUMMARY METRICS
# ============================================================

summary = pd.DataFrame({

    "Metric": [

        "Benchmark Total Return",

        "Alternative Total Return",

        "Active Total Return",

        "Benchmark Volatility",

        "Alternative Volatility",

        "Benchmark Sharpe",

        "Alternative Sharpe",

        "Benchmark Maximum Drawdown",

        "Alternative Maximum Drawdown",

        "Tracking Error",

        "Information Ratio"

    ],

    "Value": [

        benchmark_metrics[
            "Total_Return"
        ],

        alternative_metrics[
            "Total_Return"
        ],

        active_total_return,

        benchmark_metrics[
            "Annualized_Volatility"
        ],

        alternative_metrics[
            "Annualized_Volatility"
        ],

        benchmark_metrics[
            "Sharpe_Ratio"
        ],

        alternative_metrics[
            "Sharpe_Ratio"
        ],

        benchmark_metrics[
            "Maximum_Drawdown"
        ],

        alternative_metrics[
            "Maximum_Drawdown"
        ],

        annualized_tracking_error,

        information_ratio

    ]

})


summary.to_csv(

    os.path.join(
        PROJECT_02_OUTPUT,
        "risk_performance_summary.csv"
    ),

    index=False

)


# ============================================================
# 27. FINAL PROJECT 02 SUMMARY
# ============================================================

print(
    "\n" + "=" * 70
)

print(
    "PROJECT 02 COMPLETED"
)

print(
    "=" * 70
)

print(
    "\nBenchmark:"
)

print(
    "Equal Weight"
)

print(
    "\nAlternative:"
)

print(
    "Max-40 constrained portfolio"
)

print(
    "\nActive return:",
    active_total_return
)

print(
    "\nTracking error:",
    annualized_tracking_error
)

print(
    "\nInformation ratio:",
    information_ratio
)

print(
    "\nRisk contribution:"
)

print(
    risk_contribution[
        "Risk_Contribution_%"
    ]
)

print(
    "\nProject 02 outputs saved to:"
)

print(
    PROJECT_02_OUTPUT
)
